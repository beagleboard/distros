...
#ci_build
server_dir="artifacts/raw/deploy"

arch="arm64"

grab_image () {
	wget -c --directory-prefix=/tmp/ ${bmap_base_dir}/${ci_build}/${server_dir}/${file_prefix}.bmap
	if [ ! -f /tmp/${file_prefix}.bmap ] ; then
		echo "Failure to get ${bmap_base_dir}/${server_dir}/${file_prefix}.bmap"
		exit 2
	fi
	mv -v /tmp/${file_prefix}.bmap ../../bmap-temp/

	wget -c --directory-prefix=/tmp/ ${yml__base_dir}/${server_dir}/${file_prefix}.img.xz.yml.txt
	if [ ! -f /tmp/${file_prefix}.img.xz.yml.txt ] ; then
		echo "Failure to get ${yml__base_dir}/${server_dir}/${file_prefix}.img.xz.yml.txt"
		exit 2
	fi
